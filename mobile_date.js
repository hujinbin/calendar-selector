...
$(function(){
    ...
    var day=date.getDate()<10?'0'+String(date.getDate()):date.getDate();         //获取当前日期；
    //回填数据
    $('.year').text(year);
    $('.month').text(month);
    datt(year,month,'');
...
function datt(nian,yue,ri){
    var ccustomertype=window.localStorage.getItem('ccustomertype')
    var timeOut=30*24*60*60*1000
    if(Number(ccustomertype)<=1 && String(window.location.pathname).indexOf('booking.html')!== -1 ){
